...
| 六月 |                                                   |                                       |                                                        |                                                       |                                                   |                                                              |                                                              |                                                              |                                         |                                                              |                                                     |                                              |                                                              |                                                              |              <a href="#525-连续数组">525</a>              |                                           |                                                 |                                               |                                                              |     <a href="#1601-最多可达成的换楼请求数目">1601(2)</a>     |                                               |                                                              |                                                              |                                                 |                                   |                                                 |                                             |                                                   |                                                              |                                                              |                                                  |
...
| 2022.02.21 |           <a href="#838-推多米诺">838-推多米诺</a>           |    中等    | 还是能做出来的，哈哈，不过做了一个多小时，哎，这样的话是真不行啊，我现在还是有些怕面试时做不出来题。 |
...
| 2022.03.11 | <a href="#2049-统计最高分的节点数目">2049-统计最高分的节点数目</a> |  中等偏难  | 做了两个多小时，有点折磨。中等偏难，不是难在解题方法难想，而是代码实现上，注意的细节的有点多。这道题已经不像是一道二叉树的题了，更像是一道N叉树的题，而代码实现上有使用到了与图类似的邻接表。 |
| 2022.03.12 |   <a href="#590-N 叉树的后序遍历">590-N 叉树的后序遍历</a>   |    简单    | 和前天的类似，代码也几乎一致，不过这个迭代解法我也还是不想做。 |
| 2022.03.13 |     <a href="#393-UTF-8 编码验证">393-UTF-8 编码验证</a>     |    中等    | 相当于是一道字符串的题，也运用到了一些字符串和位运算的方法，但是，更好的解决方式是全部都采用位运算来解决该问题，整体来说这道题其实不是很难的，就是在遍历过程中一步一步地分析各种情况就好了。 |
| 2022.03.14 | <a href="#599-两个列表的最小索引总和">599-两个列表的最小索引总和</a> |    简单    | 这道题要看怎么实现了，如果是按照官方答案那种先将其中一个数组加入到哈希表中，再另一个哈希表，这样的话会简单很多，但是当同时遍历两个数组，再遍历长数组剩下的元素，边遍历边添加的话，这样就麻烦了很多。 |
| 2022.03.15 | <a href="#2044-统计按位或能得到最大值的子集数目">2044-统计按位或能得到最大值的子集数目</a> |    中等    | 又是做不起的一道题。现在一碰到这种求子集的题心里就一怵，就很怕。这道题有一篇不错的逐步优化的[题解](https://leetcode-cn.com/problems/count-number-of-maximum-bitwise-or-subsets/solution/by-tong-zhu-mmeu/)。 |
| 2022.03.16 | <a href="#432-全 O(1) 的数据结构">432-全 O(1) 的数据结构</a> |    困难    | 这道题和P146很像，属于同一类题。这道题并没有完全抄官方解答，了解其所采用的数据结构后，自己独立实现了，但是这并不算完全的从无到有的实现，还是参考了最基本的一些初始化的代码，不过那两个核心的方法是自己写出来的，并且最后的思路和官方的相差无几，甚至还很雷同的，哈哈哈。可是让我气愤的是，有一处bug卡了我一两个小时没有解决，结果其实就是一行代码的事。 |
| 2022.03.17 |   <a href="#720-词典中最长的单词">720-词典中最长的单词</a>   |  真不简单  | 这道题如果是使用哈希表做，还算简单，但是如果使用字典树来做，就要写很多代码，这就有些难度了。好久没写字典树了，今天不想写，现在真是越来越懒了。今天综合面试面完啦。有点点不开心的就是工资感觉低了一点点，要是再多1-2K就好了，不过我也不要太在意这一点点得失了，赶快精进技术才是正事，以后才有谈判的资本。 |
| 2022.03.18 |      <a href="#2043-简易银行系统">2043-简易银行系统</a>      |  中等偏下  | 这道题看着很吓人，但是其实是挺简单的，就是两个账户之间的金额改变而已。 |
| 2022.03.19 | <a href="#606-根据二叉树创建字符串">606-根据二叉树创建字符串</a> |    简单    | 题意很好理解，做完后确实觉得蛮简单的，不过在真正实现时，饶了些弯路，反倒显得复杂了许多。 |
| 2022.03.20 |    <a href="#2039-网络空闲的时刻">2039-网络空闲的时刻</a>    |    中等    | 太久没有做关于图的题了，又有些生疏了，不过基本思路还是有的。开始我把这当成有向图来做了，就出现了些错误，也思考得复杂了些，自己也会觉得有些奇怪。通过粗略扫描答案发现还是要用无向图来做，后续的就简单了，难一点的就是与数学相关的找规律或者说是计算公式。 |
| 2022.03.21 | <a href="#653-两数之和-IV---输入-BST">653-两数之和 IV - 输入 BST</a> |    简单    | 今天不想思考做这道题。其实是很简单，不过这就根本不用管是不是二叉搜索树，当作普通二叉树即可。(测试空格用20%代替不行，换成-可以) |
| 2022.03.22 | <a href="#2038-如果相邻两个颜色均相同则删除当前颜色">2038-如果相邻两个颜色均相同则删除当前颜色</a> |  中等偏下  | 题目不难，有点考察逻辑思维能力了，分析如何解题才是关键，难处不在代码实现上。 |
| 2022.03.23 |   <a href="#434-字符串中的单词数">434-字符串中的单词数</a>   |    简单    | 不想做今天的困难每日一题，随便挑了道简单题来做。现在还没有收到最终的offer，我想要快点去工作啊，拖得越久越觉得自己不受重视，这效率真的能把人急死。 |
| 2022.03.24 |         <a href="#661-图片平滑器">661-图片平滑器</a>         |    简单    | 就是对一个二维数组进行遍历，对九宫格内的数求和和计数而已。当然也可以采用前缀和的方式来解题——消耗多的空间，略微减少时间复杂度的系数。 |
| 2022.03.25 |         <a href="#172-阶乘后的零">172-阶乘后的零</a>         |    中等    | 这道数学题简直不想做，以后尽量少些为了做而做了。今天我收到offer了，哈哈。 |
| 2022.03.26 |           <a href="#682-棒球比赛">682-棒球比赛</a>           |    简单    | 看完题目就很容易想到这道题是一道栈的基础应用题，实现并不难，关键点还是对Java的集合类的使用，不熟练的话就很容易出错。 |
| 2022.03.27 | <a href="#2028-找出缺失的观测数据">2028-找出缺失的观测数据</a> |  中等偏下  | 算是一道数学题，有一些脑筋急转弯，只要知道分摊这个关键点就能做了，我最开始还以为需要用到回溯，一个一个数的尝试。 |
| 2022.03.28 |     <a href="#693-交替位二进制数">693-交替位二进制数</a>     |    简单    | 想到使用位运算解题并不难，逐位比较01也不难，也很好实现。不过官方这个思路没有想到，还是挺巧妙的。 |
| 2022.03.29 | <a href="#2011-执行操作后的变量值">2011-执行操作后的变量值</a> |    简单    | 为了完成每天都要做题的任务，这道题实属是滥竽充数的题，太简单了，没难度。哎，这样做是不对的啊。 |
| 2022.03.29 |  <a href="#2024-考试的最大困扰度">2024-考试的最大困扰度</a>  |    中等    | 这道题要是让我自己想方法做，八成是想不到的，只需要看一眼官解的解题思路就知道怎么做了，可是自己就是想不到。虽然思路是参考了官解，但是并没有看它的代码实现，也算是自己实现出来了。说明解题重要的还是思路啊，只要思路清晰，代码实现是真的不难的。 |
| 2022.03.30 |      <a href="#1603-设计停车系统">1603-设计停车系统</a>      |    简单    | 简单的实现类，确实这种题已经不适合目前的我做了，有些算是浪费时间了。 |
| 2022.03.30 | <a href="#1606-找到处理最多请求的服务器">1606-找到处理最多请求的服务器</a> |    困难    | 这道题没有涉及到具体的算法，更多地考察对数据结构的选取与使用，选择哪种数据结构成为了解决问题的关键点。起先只使用数组来解决此问题，由于不能在`O(1)`的时间内选择服务器，导致时间复杂度过高，因此超时了。而后参考官解选择了`有序集合 + 优先队列`的方式解决了此问题。另外，在我自己做时，还忽略了数组递增这个条件，而这正是可以选择这两种数据结构的原因。 |
| 2022.03.31 |             <a href="#728-自除数">728-自除数</a>             |    简单    | 数学题啊数学题，对数的整除和求余运算，逻辑和实现都有一点点绕，但不是很难。 |
| 2022.04.01 |       <a href="#954-二倍数对数组">954-二倍数对数组</a>       |    中等    | 这道题其实并不是很难，只是有很多需要注意的地方。特别是对于负数的一半用位运算表示是左移一位。 |
| 2022.04.02 |                             未做                             |            |                                                              |
| 2022.04.03 |                             未做                             |            |                                                              |
| 2022.04.04 |                             未做                             |            |                                                              |
| 2022.04.05 | <a href="#307-区域和检索---数组可修改">307(0)-区域和检索 - 数组可修改</a> |    中等    | 几天没做题了，生疏了太多太多了。今天这道题尝试了两种想法，最终都以超时告终。而官解的三种解法才只看了一种（分块），另外的两种解法（线段树和树状数组）还不大会，又得拖到后续再来学习了。 |
| 2022.04.06 | <a href="#307-区域和检索---数组可修改">307-区域和检索 - 数组可修改</a> |    中等    | 学习采用树状数组的做法。                                     |
| 2022.04.07 |                             未做                             |            |                                                              |
| 2022.04.08 |                             未做                             |            |                                                              |
| 2022.04.09 |           <a href="#780-到达终点">780-到达终点</a>           | 算不上困难 | 题目标记为困难，虽然代码实现也算不上困难，但是这个思路和解法是真的真的想不到的，这道题也完全是属于一道数学题。即使看了答案，也不是很清楚，它在数学上是为什么肯定最终就是这么判断的。 |
...
| 2022.06.20 | <a href="#1601-最多可达成的换楼请求数目">1601(2)-最多可达成的换楼请求数目</a> |    困难    | 真真是太久没做题了，不看做题陌生，连曾经做过的题看着都陌生，不能思考了，脑子似乎都变得迟钝了很多 |
...
模拟：指「不涉及决策和某些算法，仅仅是根据题意转化为代码」的一类题
...
|            算法            |                            字符串                            |                           一维数组                           |                           二维数组                           |                单向链表                 |                           双向链表                           |                              栈                              |                             队列                             |                        二叉树、N叉树                         |                        堆（优先队列）                        |               平衡二叉树、有序集合（TreeMap）                |                        前缀树、字典树                        | 线段树                                         | 树状数组                                       |                            有向图                            |                            无向图                            |                    其它（数学题、实现类）                    |
| :------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :-------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | ---------------------------------------------- | ---------------------------------------------- | :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: |
|          基本操作          | <a href="#434-字符串中的单词数">434</a>、<a href="#521-最长特殊序列-Ⅰ">521</a>、<a href="#537-复数乘法">537</a>、<a href="#1189-“气球”的最大数量">1189</a>、<a href="#1332-删除回文子序列">1332</a>、<a href="#2000-反转单词前缀">2000</a>、<a href="#2011-执行操作后的变量值">2011</a>、<a href="#2038-如果相邻两个颜色均相同则删除当前颜色">2038</a> | <a href="#42-接雨水">42</a>、<a href="#53-最大子数组和">53</a>、<a href="#409-最长回文串">409</a>、<a href="#539-最小时间差">539</a>、<a href="#594-最长和谐子序列">594</a>、<a href="#717-1比特与2比特字符">717</a>、<a href="#747-至少是其他数字两倍的最大数">747</a>、<a href="#1725-可以形成最大正方形的矩形数目">1725</a>、<a href="#1748-唯一元素的和">1748</a>、<a href="#2016-增量元素之间的最大差值">2016</a> | <a href="#6-Z 字形变换">6</a>、<a href="#661-图片平滑器">661</a>、<a href="#2022-将一维数组转变成二维数组">2022</a>、<a href="#1380-矩阵中的幸运数">1380</a>、<a href="#1701-平均等待时间">1701</a> | <a href="#237-删除链表中的节点">237</a> |                                                              | <a href="#682-棒球比赛">682</a>、<a href="#735-行星碰撞">735</a> |                                                              |                                                              | <a href="#215-数组中的第K个最大元素">215</a>、<a href="#239-滑动窗口最大值">239</a>、<a href="#347-前 K 个高频元素">347</a>、<a href="#1606-找到处理最多请求的服务器">1606</a> | <a href="#729-我的日程安排表 I">729</a>、<a href="#731-我的日程安排表 II">731</a>、<a href="#732-我的日程安排表 III">732</a> | <a href="#208-实现 Trie (前缀树)">208</a>、<a href="#720-词典中最长的单词">720</a> | <a href="#307-区域和检索---数组可修改">307</a> | <a href="#307-区域和检索---数组可修改">307</a> |                                                              |        <a href="#1791-找出星型图的中心节点">1791</a>         | <a href="#172-阶乘后的零">172</a>、<a href="#258-各位相加">258</a>、<a href="#357-统计各位数字都不同的数字个数">357</a>、<a href="#504-七进制数">504</a>、<a href="#553-最优除法">553</a>、<a href="#728-自除数">728</a>、<a href="#780-到达终点">780</a>、<a href="#1342-将数字变成 0 的操作次数">1342</a>、<a href="#1447-最简分数">1447</a>、<a href="#1603-设计停车系统">1603</a>、<a href="#2028-找出缺失的观测数据">2028</a>、<a href="#2043-简易银行系统">2043</a> |
|           哈希表           |         <a href="#884-两句话中的不常见单词">884</a>          | <a href="#217-存在重复元素">217</a>、<a href="#219-存在重复元素 II">219</a>、<a href="#409-最长回文串">409</a>、<a href="#594-最长和谐子序列">594</a>、<a href="#599-两个列表的最小索引总和">599</a>、<a href="#720-词典中最长的单词">720</a>、<a href="#804-唯一摩尔斯密码词">804</a>、<a href="#954-二倍数对数组">954</a>、<a href="#2006-差的绝对值为 K 的数对数目">2006</a> | <a href="#391-完美矩形">391</a>、<a href="#554-砖墙">554</a>、<a href="#1001-网格照明">1001</a> |   <a href="#146-LRU 缓存机制">146</a>   | <a href="#146-LRU 缓存机制">146</a>、<a href="#432-全 O(1) 的数据结构">432</a> |                                                              |                                                              |                                                              |            <a href="#2034-股票价格波动">2034</a>             |            <a href="#2034-股票价格波动">2034</a>             |                                                              |                                                |                                                |                                                              |                                                              |                                                              |
|            递归            |                                                              |                                                              |                                                              |                                         |                                                              |                                                              |                                                              | <a href="#101-对称二叉树">101</a>、<a href="#124-二叉树中的最大路径和">124</a>、<a href="#145-二叉树的后序遍历">145</a>、<a href="#226-翻转二叉树">226</a>、<a href="#236-二叉树的最近公共祖先">236</a>、<a href="#589-N 叉树的前序遍历">589</a>、<a href="#590-N 叉树的后序遍历">590</a>、<a href="#606-根据二叉树创建字符串">606</a>、<a href="#653-两数之和-IV---输入-BST">653</a> |                                                              |                                                              |                                                              |                                                |                                                |                                                              |                                                              | <a href="#70-爬楼梯">70</a>、<a href="#509-斐波那契数">509</a>、<a href="#1137-第 N 个泰波那契数">1137</a>、<a href="#1414-和为 K 的最少斐波那契数字数目">1414</a> |
|            迭代            |                                                              |                                                              |              <a href="#463-岛屿的周长">463</a>               |                                         |                                                              |                                                              |                                                              | <a href="#101-对称二叉树">101</a>、<a href="#145-二叉树的后序遍历">145</a>、<a href="#589-N 叉树的前序遍历">589</a>、<a href="#590-N 叉树的后序遍历">590</a>、<a href="#606-根据二叉树创建字符串">606</a> |                                                              |                                                              |                                                              |                                                |                                                |                                                              |                                                              |                                                              |
|            回溯            |              <a href="#93-复原 IP 地址">93</a>               | <a href="#46-全排列">46</a>、<a href="#47-全排列 II">47</a>、<a href="#494-目标和">494</a>、<a href="#2044-统计按位或能得到最大值的子集数目">2044</a> | <a href="#980-不同路径 III">980</a>、<a href="#1219-黄金矿工">1219</a>、<a href="#1601-最多可达成的换楼请求数目">1601</a> |                                         |                                                              |                                                              |                                                              |                                                              |                                                              |                                                              |                                                              |                                                |                                                |                                                              |                                                              |                                                              |
|          分治算法          |         <a href="#1763-最长的美好子字符串">1763</a>          |              <a href="#53-最大子数组和">53</a>               |                                                              |  <a href="#23-合并K个升序链表">23</a>   |                                                              |                                                              |                                                              |                                                              |                                                              |                                                              |                                                              |                                                |                                                |                                                              |                                                              |                                                              |
|          贪心算法          |           <a href="#1405-最长快乐字符串">1405</a>            | <a href="#45-跳跃游戏 II">45</a>、<a href="#53-最大子数组和">53</a>、<a href="#55-跳跃游戏">55</a>、<a href="#334-递增的三元子序列">334</a> |                                                              |                                         |                                                              |                                                              |                                                              |                                                              |              <a href="#630-课程表 III">630</a>               |                                                              |                                                              |                                                |                                                |                                                              |                                                              |    <a href="#1414-和为 K 的最少斐波那契数字数目">1414</a>    |
|          动态规划          | <a href="#5-最长回文子串">5</a>、<a href="#72-编辑距离">72</a>、<a href="#516-最长回文子序列">516</a>、<a href="#1143-最长公共子序列">1143</a> | <a href="#42-接雨水">42</a>、<a href="#53-最大子数组和">53</a>、<a href="#121-买卖股票的最佳时机">121</a>、<a href="#122-买卖股票的最佳时机 II">122</a>、<a href="#123-买卖股票的最佳时机 III">123</a>、<a href="#188-买卖股票的最佳时机 IV">188</a>、<a href="#198-打家劫舍">198</a>、<a href="#213-打家劫舍 II">213</a>、<a href="#300-最长递增子序列">300</a>、<a href="#322-零钱兑换">322</a>、<a href="#518-零钱兑换 II">518</a>、<a href="#673-最长递增子序列的个数">673</a>、<a href="#740-删除并获得点数">740</a>、<a href="#746-使用最小花费爬楼梯">746</a>、<a href="#2100-适合打劫银行的日子">2100</a> | <a href="#62-不同路径">62</a>、<a href="#63-不同路径 II">63</a>、<a href="#64-最小路径和">64</a>、<a href="#494-目标和">494</a>、<a href="#688-骑士在棋盘上的概率">688</a>、<a href="#1444-切披萨的方案数">1444</a> |                                         |                                                              |                                                              |                                                              |             <a href="#337-打家劫舍 III">337</a>              |                                                              |                                                              |                                                              |                                                |                                                |                                                              |                                                              | <a href="#70-爬楼梯">70</a>、<a href="#509-斐波那契数">509</a>、<a href="#1137-第 N 个泰波那契数">1137</a>、<a href="#1220-统计元音字母序列的数目">1220</a>、<a href="#1411-给 N x 3 网格图涂色的方案数">1411</a> |
|        状态压缩 DP         |                                                              | <a href="#1994-好子集的数目">1994</a>、<a href="#2044-统计按位或能得到最大值的子集数目">2044</a> |                                                              |                                         |                                                              |                                                              |                                                              |                                                              |                                                              |                                                              |                                                              |                                                |                                                |             <a href="#1494-并行课程 II">1494</a>             |                                                              |                                                              |
|      深度优先搜索 DFS      |                                                              |                                                              | <a href="#200-岛屿数量">200</a>、<a href="#463-岛屿的周长">463</a>、<a href="#695-岛屿的最大面积">695</a>、<a href="#827-最大人工岛">827</a>、<a href="#1020-飞地的数量">1020</a>、<a href="#1254-统计封闭岛屿的数目">1254</a>、<a href="#1706-球会落何处">1706</a>、<a href="#1905-统计子岛屿">1905</a> |                                         |                                                              | <a href="#200-岛屿数量">200</a>、<a href="#695-岛屿的最大面积">695</a> |                              ——                              | <a href="#563-二叉树的坡度">563</a>、<a href="#2049-统计最高分的节点数目">2049</a> |                                                              |                                                              |                <a href="#472-连接词">472</a>                 |                                                |                                                | <a href="#207-课程表">207</a>、<a href="#210-课程表 II">210</a>、<a href="#1462-课程表 IV">1462</a> |                                                              |                                                              |
|      广度优先搜索 BFS      |                                                              |                                                              |                                                              |                                         |                                                              |                                                              | <a href="#200-岛屿数量">200</a>、<a href="#695-岛屿的最大面积">695</a>、<a href="#864-获取所有钥匙的最短路径">864</a>、<a href="#994-腐烂的橘子">994</a>、<a href="#1020-飞地的数量">1020</a>、<a href="#1254-统计封闭岛屿的数目">1254</a>、<a href="#1765-地图中的最高点">1765</a>、<a href="#1905-统计子岛屿">1905</a> | <a href="#103-二叉树的锯齿形层序遍历">103</a>、<a href="#653-两数之和-IV---输入-BST">653</a> |                                                              |                                                              |                                                              |                                                |                                                | <a href="#207-课程表">207</a>、<a href="#210-课程表 II">210</a>、<a href="#1462-课程表 IV">1462</a> | <a href="#1368-使网格图至少有一条有效路径的最小代价">1368</a>、<a href="#2039-网络空闲的时刻">2039</a>、<a href="#2045-到达目的地的第二短时间">2045</a> |                                                              |
|         普里姆算法         |                                                              |                                                              |                                                              |                                         |                                                              |                                                              |                                                              |                                                              |                                                              |                                                              |                                                              |                                                |                                                |                                                              |                                                              |                                                              |
|       克鲁斯卡尔算法       |                                                              |                                                              |                                                              |                                         |                                                              |                                                              |                                                              |                                                              |                                                              |                                                              |                                                              |                                                |                                                |                                                              |                                                              |                                                              |
...
|     前缀和 + 二分查找      |        | <a href="#209-长度最小的子数组">209</a>、<a href="#1712-将数组分成三个子数组的方案数">1712</a> |                                               |          |      |                                           |              |                                                              |
...
|   滑动窗口    | <a href="#3-无重复字符的最长子串">3</a>、<a href="#1763-最长的美好子字符串">1763</a>、<a href="#2024-考试的最大困扰度">2024</a> | <a href="#209-长度最小的子数组">209</a>、<a href="#219-存在重复元素 II">219</a>、<a href="#1984-学生分数的最小差值">1984</a> |                                             |                                                              | <a href="#220-存在重复元素 III">220</a> |                                                              |
|    单调栈     |                                                              |                                                              |                                             | <a href="#42-接雨水">42</a>、<a href="#739-每日温度">739</a>、<a href="#2104-子数组范围和">2104</a> |                                         |                                                              |
|   单调队列    |                                                              |            <a href="#239-滑动窗口最大值">239</a>             |                                             |                                                              |                                         |                                                              |
| 双指针/三指针 | <a href="#838-推多米诺">838</a>、<a href="#917-仅仅反转字母">917</a> | <a href="#11-盛最多水的容器">11</a>、<a href="#15-三数之和">15</a>（排序 + 双指针）、<a href="#42-接雨水">42</a>（感觉像是对动态规划的优化）、<a href="#825-适龄的朋友">825</a>、<a href="#1712-将数组分成三个子数组的方案数">1712</a>（三指针） |                                             |                                                              |                                         |                                                              |
...
|  二进制枚举   |                                                              |     <a href="#1601-最多可达成的换楼请求数目">1601(2)</a>     |                                             |                                                              |                                         |                                                              |
...
| [53. 最大子数组和](https://leetcode-cn.com/problems/maximum-subarray/) | 数组 | 动态规划、贪心、分治 | 简单 | 2022.02.16 | 2022.05.17 |  | 是 | 是 | 是 | B | <a href="#53-最大子数组和">53</a> |
...
| [146. LRU 缓存机制](https://leetcode-cn.com/problems/lru-cache/) | 双向链表 | 哈希表 | 中等 |  2021.10.15  |  2021.10.16  |    2     | 否                 |             否             |               是               |               A                |          <a href="#146-LRU 缓存机制">146</a>          |
...
| [172. 阶乘后的零](https://leetcode-cn.com/problems/factorial-trailing-zeroes/) | 数学题 | 基本操作 | 中等 | 2022.03.25 | 2022.03.25 | 1 | 否 | 否 | 否 | C | <a href="#172-阶乘后的零">172</a> |
...
| [209. 长度最小的子数组](https://leetcode-cn.com/problems/minimum-size-subarray-sum/) | 数组 | 前缀和 + 二分查找、滑动窗口 | 中等 | 2022.02.20 | 2022.02.20 | 1 | 是 | 是 | 否 | B | <a href="#209-长度最小的子数组">209</a> |
...
| [258. 各位相加](https://leetcode-cn.com/problems/add-digits/) | 数学题 | 基本操作 | 简单 | 2022.03.03 | 2022.03.03 | 1 | 是 | 是 | 是 | D | <a href="#258-各位相加">258</a> |
...
| [307. 区域和检索 - 数组可修改](https://leetcode-cn.com/problems/range-sum-query-mutable/) | 实现类 | 分块处理、线段树、树状数组 | 中等 | 2022.04.05 | 2022.04.06 | 1 | 否 | 否 | 否 | B | <a href="#307-区域和检索---数组可修改">307</a> |
...
| [357. 统计各位数字都不同的数字个数](https://leetcode-cn.com/problems/count-numbers-with-unique-digits/) | 数学题 | 基本操作 | 中等 | 2022.04.11 | 2022.04.12 | 1 | 是 | 是 | 是 | C | <a href="#357-统计各位数字都不同的数字个数">357</a> |
...
| [432. 全 O(1) 的数据结构](https://leetcode-cn.com/problems/all-oone-data-structure/) | 双向链表 | 哈希表 | 困难 | 2022.03.16 | 2022.03.16 | 1 | 否 | 否 | 否 | A | <a href="#432-全 O(1) 的数据结构">432</a> |
| [434. 字符串中的单词数](https://leetcode-cn.com/problems/number-of-segments-in-a-string/) | 字符串 | 基本操作 | 简单 | 2022.03.23 | 2022.03.23 | 1 | 是 | 是 | 是 | D | <a href="#434-字符串中的单词数">434</a> |
...
| [504. 七进制数](https://leetcode-cn.com/problems/base-7/) | 数学题 | 基本操作 | 简单 | 2022.03.07 | 2022.03.07 | 1 | 否 | 否 | 否 | D | <a href="#504-七进制数">504</a> |
...
| [553. 最优除法](https://leetcode-cn.com/problems/optimal-division/) | 数学题 | 基本操作 | 中等 | 2022.02.27 | 2022.02.27 | 1 | 否 | 否 | 否 | C | <a href="#553-最优除法">553</a> |
| [554. 砖墙](https://leetcode-cn.com/problems/brick-wall/) | 数组 | 哈希表 | 中等 | 2022.03.02 | 2022.03.02 | 1 | 是 | 是 | 否 | C | <a href="#554-砖墙">554</a> |
...
| [599. 两个列表的最小索引总和](https://leetcode-cn.com/problems/minimum-index-sum-of-two-lists/) | 数组 | 哈希表 | 简单 | 2022.03.14 | 2022.03.14 | 1 | 是 | 是 | 是 | C | <a href="#599-两个列表的最小索引总和">599</a> |
| [606. 根据二叉树创建字符串](https://leetcode-cn.com/problems/construct-string-from-binary-tree/) | 二叉树 | 递归、迭代 | 简单 | 2022.03.19 | 2022.03.19 | 1 | 是 | 是 | 是 | C | <a href="#606-根据二叉树创建字符串">606</a> |
...
| [653. 两数之和 IV - 输入 BST](https://leetcode-cn.com/problems/two-sum-iv-input-is-a-bst/) | 二叉树 | 递归 | 简单 | 2022.03.21 | 2022.03.21 | 1 | 否 | 否 | 否 | C | <a href="#653-两数之和-IV---输入-BST">653</a> |
| [661. 图片平滑器](https://leetcode-cn.com/problems/image-smoother/) | 数组 | 基本操作 | 简单 | 2022.03.24 | 2022.03.24 | 1 | 是 | 是 | 否 | D | <a href="#661-图片平滑器">661</a> |
...
| [682. 棒球比赛](https://leetcode-cn.com/problems/baseball-game/) | 栈 | 基本操作 | 简单 | 2022.03.26 | 2022.03.26 | 1 | 是 | 是 | 是 | C | <a href="#682-棒球比赛">682</a> |
...
| [693. 交替位二进制数](https://leetcode-cn.com/problems/binary-number-with-alternating-bits/) | 其它 | 位运算 | 简单 | 2022.03.28 | 2022.03.28 | 1 | 是 | 是 | 否 | D | <a href="#693-交替位二进制数">693</a> |
...
| [717. 1比特与2比特字符](https://leetcode-cn.com/problems/1-bit-and-2-bit-characters/) | 数组 | 基本操作 | 简单 | 2022.02.20 | 2020.02.20 | 1 | 是 | 是 | 否 | D | <a href="#717-1比特与2比特字符">717</a> |
| [720. 词典中最长的单词](https://leetcode-cn.com/problems/longest-word-in-dictionary/) | 字符串数组 | 哈希表、字典树 | 简单 | 2022.03.17 | 2022.03.17 | 1 | 是 | 是 | 否 | C | <a href="#720-词典中最长的单词">720</a> |
| [728. 自除数](https://leetcode-cn.com/problems/self-dividing-numbers/) | 数学题 | 基本操作 | 简单 | 2022.03.31 | 2022.03.31 | 1 | 是 | 是 | 否 | C | <a href="#728-自除数">728</a> |
...
| [780. 到达终点](https://leetcode-cn.com/problems/reaching-points/) | 数学题 | 反向计算 | 困难 | 2022.04.09 | 2022.04.09 | 1 | 否 | 否 | 否 | B | <a href="#780-到达终点">780</a> |
| [804. 唯一摩尔斯密码词](https://leetcode-cn.com/problems/unique-morse-code-words/) | 数组 | 哈希表 | 简单 | 2022.04.10 | 2022.04.10 | 1 | 是 | 是 | 否 | D | <a href="#804-唯一摩尔斯密码词">804</a> |
...
| [838. 推多米诺](https://leetcode-cn.com/problems/push-dominoes/) | 字符串 | 双指针 | 中等 | 2022.02.21 | 2022.02.21 | 1 | 是 | 是 | 是 | B | <a href="#838-推多米诺">838</a> |
...
| [954. 二倍数对数组](https://leetcode-cn.com/problems/array-of-doubled-pairs/) | 数组 | 哈希表 + 排序 | 中等 | 2022.04.01 | 2022.04.01 | 1 | 是 | 是 | 是 | B | <a href="#954-二倍数对数组">954</a> |
...
| [1380. 矩阵中的幸运数](https://leetcode-cn.com/problems/lucky-numbers-in-a-matrix/) | 数组 | 基本操作 | 简单 | 2022.02.15 | 2022.02.15 | 1 | 是 | 是 | 是 | C | <a href="#1380-矩阵中的幸运数">1380</a> |
...
| [1601. 最多可达成的换楼请求数目](https://leetcode-cn.com/problems/maximum-number-of-achievable-transfer-requests/) | 数组、图 | 回溯、二进制枚举 | 困难 | 2022.02.28 | 2022.06.20 | 2 | 否 | 否 | 否 | A | <a href="#1601-最多可达成的换楼请求数目">1601</a> |
...
| [1606. 找到处理最多请求的服务器](https://leetcode-cn.com/problems/find-servers-that-handled-most-number-of-requests/) | 有序集合 + 优先队列 | 基本操作 | 困难 | 2022.03.30 | 2022.03.30 | 1 | 否 | 否 | 否 | B | <a href="#1606-找到处理最多请求的服务器">1606</a> |
...
| [1791. 找出星型图的中心节点](https://leetcode-cn.com/problems/find-center-of-star-graph/) | 图 | 数组基本操作 | 简单 | 2022.02.18 | 2022.02.18 | 1 | 是 | 是 | 是 | D | <a href="#1791-找出星型图的中心节点">1791</a> |
...
| [2011. 执行操作后的变量值](https://leetcode-cn.com/problems/final-value-of-variable-after-performing-operations/) | 字符串 | 基本操作 | 简单 | 2022.03.29 | 2022.03.29 | 1 | 是 | 是 | 是 | D | <a href="#2011-执行操作后的变量值">2011</a> |
...
| [2024. 考试的最大困扰度](https://leetcode-cn.com/problems/maximize-the-confusion-of-an-exam/) | 字符串 | 滑动窗口 | 中等 | 2022.03.29 | 2022.03.29 | 1 | 否 | 否 | 否 | B | <a href="#2024-考试的最大困扰度">2024</a> |
...
| [2038. 如果相邻两个颜色均相同则删除当前颜色](https://leetcode-cn.com/problems/remove-colored-pieces-if-both-neighbors-are-the-same-color/) | 字符串 | 基本操作 | 中等 | 2022.03.22 | 2022.03.22 | 1 | 是 | 是 | 是 | C | <a href="#2038-如果相邻两个颜色均相同则删除当前颜色">2038</a> |
| [2039. 网络空闲的时刻](https://leetcode-cn.com/problems/the-time-when-the-network-becomes-idle/) | 无向图 | BFS | 中等 | 2022.03.20 | 2022.03.20 | 1 | 是 | 是 | 否 | B | <a href="#2039-网络空闲的时刻">2039</a> |
| [2043. 简易银行系统](https://leetcode-cn.com/problems/simple-bank-system/) | 实现类 | 基本操作 | 中等 | 2022.03.18 | 2022.03.18 | 1 | 是 | 是 | 是 | C | <a href="#2043-简易银行系统">2043</a> |
| [2044. 统计按位或能得到最大值的子集数目](https://leetcode-cn.com/problems/count-number-of-maximum-bitwise-or-subsets/) | 数组 | 位运算、回溯、状态压缩 DP | 中等 | 2022.03.15 | 2022.03.15 | 1 | 否 | 否 | 否 | B | <a href="#2044-统计按位或能得到最大值的子集数目">2044</a> |
...
| [2049. 统计最高分的节点数目](https://leetcode-cn.com/problems/count-nodes-with-the-highest-score/) | 二叉树 | DFS | 中等 | 2022.03.11 | 2022.03.11 | 1 | 是 | 是 | 是 | B+ | <a href="#2049-统计最高分的节点数目">2049</a> |
...
> 页内跳转：#后 字符省略，空格变为 - ，即将所有空格变为 -，才能在GitHub上页内跳转
>
> <a href="#"></a>
